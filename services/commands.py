...
def switch_on(bot, update, job_queue, chat_data):
    ...
    if mq.get_user_settings(msg.chat_id)[base_config.NOTIFICATIONS]:
        msg.reply_text(messages.ALREADY_ON_TEXT, parse_mode=messages.MARKDOWN)
        return
...
def switch_off(bot, update, job_queue, chat_data):
    ...
    if not mq.get_user_settings(msg.chat_id)[base_config.NOTIFICATIONS]:
        msg.reply_text(messages.ALREADY_OFF_TEXT, parse_mode=messages.MARKDOWN)
        return
...
def set_interval(bot, update, args, chat_data, job_queue):
    ...
    try:
        ...
        msg.reply_text(messages.SET_INTERVAL_HELP_TEXT, parse_mode=messages.MARKDOWN)
...
def set_threshold(bot, update, args):
    ...
    try:
        ...
        msg.reply_text(messages.SET_THRESHOLD_HELP_TEXT, parse_mode=messages.MARKDOWN)
    except OverflowError:
